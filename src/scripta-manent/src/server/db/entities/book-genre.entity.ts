...
/**
 * Explicit junction entity for the `book_genres` table.
 *
 ...
 * full, correct metadata (including `databaseName`) for `book_genres`, making
 * pagination + ManyToMany join safe even under webpack bundling.
 *
 * NOTE: do NOT define `@JoinTable` on both sides — the owning side
 * (@JoinTable on BookEntity.genres) manages the junction. This entity only
...
export class BookGenreEntity {
  ...
  @PrimaryColumn({ name: 'genre_id', type: 'uuid' })
  genreId!: string;
  ...
  @ManyToOne(() => GenreEntity, { eager: false, onDelete: 'CASCADE' })
